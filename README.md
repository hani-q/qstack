...
| [`qstack-loop-no-nonsense`](skills/qstack-loop-no-nonsense/) | Explicit | Execute the plan exactly. Stop before any deviation, keep `execution.md` current, and ask how much adversarial review to run. With `--orchestrator` and `--workers` it routes each card's subagent to a worker model, honouring the card's own `model` hint when the tier allows it. Progress is one line per board transition, and every card it names carries its title and a board link. |
| [`qstack-loop-trequartista`](skills/qstack-loop-trequartista/) | Explicit | Execute the plan with controlled creative freedom. Preserve its intent, record every adaptation, and ask the same review-depth question. Same progress voice, card references, and report template. Takes the same `--orchestrator` and `--workers` routing as no-nonsense. |
...
| [`qstack-choose-model`](skills/qstack-choose-model/) | Auto | Pick the model and reasoning effort for one piece of work. Discovers the reachable catalogue from the CLI's own proxy settings, scores it with the Artificial Analysis Data API when a key is present, tiers it strong / standard / cheap minus the orchestrator, classifies the task from its metadata, probes the pick, and returns one tuple with the reason. Called by breakdown, both loops and slalom. |
...
└── skills/                          ← the layout skills.sh discovers
    ...
    ├── ...                          ← one directory per skill, 31 in total
    ├── qstack-how/
    │   ├── SKILL.md
